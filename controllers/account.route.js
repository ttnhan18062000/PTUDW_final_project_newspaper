...
router.get('/reset-password', async function(req, res) {
  res.render('../views/vmAccount/reset-password.hbs', { email: req.query.email });
})
...
router.get('/profile', ensureAuth, async function(req, res) {
  ...
  return res.render('../views/vmAccount/profile.hbs', { accountDetail });
});
...
router.post('/profile', async function(req, res) {
  ...
  if (emailChange === "1") {
    ...
    let accountDetail = await accountModel.findAccountDetailByID(account.ID);
    accountDetail.DOB = moment(accountDetail.DOB).format("DD/MM/YYYY");
    return res.render('../views/vmAccount/profile.hbs', { message: "Your Account Infomation has been changed", accountDetail });
  }
